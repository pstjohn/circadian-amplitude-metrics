...
class Odesol(object):
    """
    Class to calculate basic properties of a limit cycle oscillator
    using casadi 1.5.0 function calls. Many properties of this class
    have been replaced with pure python function calls (i.e.,
    self.roots) for code portability at the expense of runtime speed.
    """
        
    def __init__(self, model, paramset, y0):
        """
        Sets up Odesol with the required information
    ...
    def solveBVP(self, method='scipy', backup='casadi'):
        """
    ...
    def roots(self):
        ...
        """
        if ~hasattr(self, 'lc'): self.limitCycle()
        deriv = self.interp_sol(self.ts, self.dydt(self.sol))
        splines = deriv.splines

        self.Ymax = np.zeros(self.NEQ)
        self.Ymin = np.zeros(self.NEQ)
        self.Tmax = np.zeros(self.NEQ)
        self.Tmin = np.zeros(self.NEQ)
        
        for i in xrange(self.NEQ):
            roots = splines[i].roots()
            if (len(roots) < 2) & (np.abs(deriv(0)[i]) < 1E-6):
                roots = np.concatenate([np.zeros(1),
                                        splines[i].roots()])

            extrema = self.lc(roots)[:,i]
            self.Ymax[i] = extrema.max()
            self.Ymin[i] = extrema.min()
            self.Tmax[i] = roots[extrema.argmax()]
            self.Tmin[i] = roots[extrema.argmin()]
    ...
    def phase_of_point(self, point, error=False, tol=1E-3):
        ...
        for i in xrange(100):
            ...
            roots_class = Odesol(dist_model, self.paramset, cat_y0)
